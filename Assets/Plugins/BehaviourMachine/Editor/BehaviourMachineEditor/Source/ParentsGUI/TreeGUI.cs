...
namespace BehaviourMachineEditor
{
    ...
    public class TreeGUI : ParentBehaviourGUI
    {
                ...
                if (current.type == EventType.Repaint)
                {
                    ...
                    else
                    {
                        ...
                        if (node.IsMatchSearch)
                        {
                            ...
                            pingRect.yMin += 1f;
                            pingRect.yMax -= 1f;
                            DrawMatchSearch(pingRect, guiContent);
                            this.Repaint();
                        }
                    ...
                    // Draw insertion?
                    if (m_DropDrag.insertion == node)
                    { // if (insertInside && (mousePosition.y >= rect.y) && (mousePosition.y <= rect.y + 2f)) {
        ...
        private void DrawMatchSearch(Rect rect, GUIContent guiContent)
        {
            var oldColor = GUI.color;
            var pingStyle = TreeGUI.s_Styles.ping;
            pingStyle.Draw(rect, guiContent, false, false, false, false);
            GUI.color = oldColor;
        }
